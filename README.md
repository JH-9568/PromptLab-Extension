...
로그에는 프롬프트 전문을 저장하지 않습니다. 저장되는 정보는 익명 사용자 ID, 세션 ID, 작업 카테고리, 개선 전후 분석 메타데이터, 개선 유형 및 개선 이유 메타데이터, 프롬프트 해시, 글자 수, 만족도 점수 등 제한적인 익명 메타데이터입니다.
...
PromptLab logs do not store full prompt text. Stored data is limited to anonymous metadata such as anonymous user ID, session ID, task category, before/after analysis metadata, improvement type/reason metadata, prompt hashes, character lengths, and satisfaction ratings.
